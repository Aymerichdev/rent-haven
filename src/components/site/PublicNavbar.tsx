...
export function PublicNavbar() {
  ...
  const linkBase = "px-3 py-2 text-sm font-medium text-neutral-700 hover:text-[#0096fa] transition-colors";
  const linkActive = "text-[#0096fa]";

  return (
    <header className="sticky top-0 z-40 w-full border-b border-neutral-200 bg-white">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link to="/" className="flex items-center gap-2">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#0096fa] text-white">
            <Home className="h-5 w-5" />
          </div>
          <span className="font-display text-lg font-bold tracking-tight text-neutral-900">
            BR<span className="text-[#0096fa]"> Internacional</span>
          </span>
        </Link>
        ...
        <div className="flex items-center gap-3">
          <button aria-label="Buscar" className="hidden text-neutral-700 hover:text-[#0096fa] md:inline-flex">
          ...
          {!user ? (
            <>
              <Link to="/login" className="text-sm font-medium text-neutral-800 hover:text-[#0096fa]">
                Iniciar sesión
              </Link>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-2 rounded-full border border-neutral-200 bg-white px-2 py-1.5 hover:border-[#0096fa]">
                  <Avatar className="h-7 w-7">
                    <AvatarFallback className="bg-[#0096fa] text-xs text-white">
      ...
      <div className="mx-auto grid max-w-7xl gap-8 px-6 py-12 md:grid-cols-4">
        <div>
          <div className="flex items-center gap-2">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#0096fa] text-white">
